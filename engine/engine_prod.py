...
class Engine(EngineBase):
    ...
    def set_material(self, material, detail):
        ...
        if detail['type'] == 'fabric':
            FabricWorker.create_fabric_multi_material(material, scale)
            FabricWorker.collar_seam_multi_material(material)
        if detail['type'] == 'preset':
            FabricWorker.create_fabric_multi_material(material, scale)
            FabricWorker.collar_seam_multi_material(material)
            PlasticWorker.create_img_button_material(detail['button_texture'])
            LabelWorker.label_seam_multi_material(detail['label_texture'])
        if detail['type'] == 'plastic':
            pass
        if detail['type'] == 'label':
            LabelWorker.label_seam_multi_material(material)
            pass
